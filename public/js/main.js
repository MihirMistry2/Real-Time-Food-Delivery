...
function initAdmin(socket) {
  ...
  function orderItems(items) {
    var itemLists = Object.values(items);
    console.log('PARS:', itemLists);
    return itemLists.map(function (menuItem) {
...
var cartCounter = document.getElementById('cartCouner');
...
var _iterator = _createForOfIteratorHelper(addToCartButton),
    _step;
...
var orderLists = document.querySelector('#hiddenOrder');
var order = orderLists ? orderLists.value : null;
...
function updateOrderStatus(order) {
  ...
  try {
    for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
      ...
      var dataName = _stat.dataset.status;

      if (stepCompleted) {
        _stat.classList.add('text-muted');
      }

      if (dataName === order.status) {
        stepCompleted = false;

        if (_stat.nextElementSibling) {
          _stat.nextElementSibling.classList.add('text-success', 'font-weight-bold');
        }
      }
    }
  } catch (err) {
...
updateOrderStatus(order); // socket io

var socket = io();
initAdmin(socket);

if (order) {
  socket.emit('data', "order_".concat(order._id));
}
...
socket.on('orderUpdated', function (data) {
  var updateOrders = _objectSpread({}, order); // let date = new Date.now();
